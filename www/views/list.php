					...
					<tr id="video_<?= $f['id'] ?>">
						<td>
							<?php if ($file->get_relative_downloads_folder()): ?>
								<a href="<?= rawurlencode($file->get_relative_downloads_folder()) . '/' . rawurlencode($f["name"]) ?>"
									download><?= $f["name"] ?></a>
							<?php else: ?>
								<?= $f["name"] ?>
							<?php endif; ?><br>
							<br>
							<a target="_blank"
								href="<?= $f['info']['url'] ?>"><?= htmlspecialchars(@$f["info"]["details_json"]["title"]) ?></a>
						</td>
						<td><?= $f["size"] ?></td>
						<td><?= @$f["info"]["username"] ?></td>
						<td><?= @$f["info"]["details_json"]["cutFrom"] . '-' . @$f["info"]["details_json"]["cutTo"] ?></td>
						<td>
							<a href="<?= "info.php?url=" . urlencode($f['info']['url']) ?>"
								class="btn btn-info btn-sm pull-right">Info</a>
							<a href="<?= "list.php?delete=" . sha1($f['name']) ?>"
								class="btn btn-danger btn-sm pull-right">Delete</a>
						</td>
					</tr>
				<?php endforeach; ?>
			</tbody>
