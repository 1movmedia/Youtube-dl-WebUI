				...
				<ul class="navbar-nav mr-auto">
					...
					if ($session->is_logged_in() && isset($file)) {
						...
						<li class="nav-item mx-1"><a class="nav-link" href="./">
								<span class="align-text-bottom"><svg xmlns="http://www.w3.org/2000/svg" width="16"
						...
						<li class="nav-item mx-1"><a class="nav-link" href="./list.php">
								...
								<?php
								// List of files
								$filesCount = count(iterator_to_array($file->listFiles()));
								if ($filesCount < 1) {
									echo '							List of files';
								} else {
						...
						<?php else: ?>
							<li class="nav-item mx-1 dropdown">
								<a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button"
									data-bs-toggle="dropdown" aria-expanded="false">TSV Feed<span class="caret"></span></a>
								<ul class="dropdown-menu" aria-labelledby="navbarDropdown">

									<?php foreach ($config['targets'] as $target): ?>
										<li><a class="dropdown-item"
												href="/dump.php?target=<?= $target ?>&mark_exported=n&remove_marked=n"><?= $target ?></a>
										</li>
									<?php endforeach; ?>
								</ul>
							</li>
						...
						if ($file->is_log_enabled()) {
							...
							<li class="nav-item mx-1"><a class="nav-link" href="./logs.php">
									<span class="align-text-bottom"><svg xmlns="http://www.w3.org/2000/svg" width="16"
											height="16" fill="currentColor" class="bi bi-journals" viewBox="0 0 16 16">
											<path
									...
									$cost = number_format($filesCount * 0.03, 2);
									if ($filesCount < 1) {
										echo '						Logs';
									} else {
										echo '						<b>Logs</b> (<span class="log_count">' . $filesCount . ' / $' . $cost . '</span>)';
									}
									unset($filesCount);
									?>
								</a></li>
						...
						<li class="nav-item mx-1 dropdown">
							...
							<ul class="dropdown-menu" aria-labelledby="navbarDropdown">
								...
								<?php if (!empty($background_jobs)): ?>
									<?php foreach (array_filter($background_jobs, function ($job) {
										return !empty($job['tree']); }) as $job): ?>
										<li>
			...
			<ul class="navbar-nav mr-auto justify-content-end">
				...
				if ($session->is_logged_in()) {
					...
					<li class="nav-item"><a class="nav-link" href="./logout.php">
							<span class="align-text-bottom"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
